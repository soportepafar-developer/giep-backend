...
use App\Dto\Evaluacion\OpcionesEvaluacionOutPutDto;
 ...
 */
class OpcionesEvaluacionRepository extends ServiceEntityRepository
{
    private $security;
    public function __construct(ManagerRegistry $registry,Security $security)
    {
        $this->security = $security;
        parent::__construct($registry, OpcionesEvaluacion::class);
    }
    ...
    public function findAllPage($data){
        ...
        foreach($paginator as $clave=>$valor){
            $opcionesDto =new OpcionesEvaluacionOutPutDto();
            ...
            "IdEvaluacion"=>$valor->getIdPregunta()->getIdEvaluacion()->getId(),"EvaluacionNombre"=>$valor->getIdPregunta()->getIdEvaluacion()->getNombre()):[];
       ...
       return array("count"=>count($paginatorTotalCount),"data"=>$dataopciones);
          ...
          /**
     * Create OpcionesEvaluacion.
     */
    public function post($data,$validator,$helper): JsonResponse  {
        $entityManager = $this->getEntityManager();
        $entity=$helper->setParametersToEntity(new OpcionesEvaluacion(),$data);
    ...
    public function findEvaluacionByCaptura($id){
        ...
        foreach($moduleData as $valor){
            ...
            $dataevaluacion[]=$valor->getIdPregunta()->getIdEvaluacion()->getId(),
            $dataevaluacion[]=$valor->getIdPregunta()->getIdEvaluacion()->getNombre(),
    ...
    /**
     * Update OpcionesEvaluacion.
     */
    public function put($data,$id,$validator,$helper): JsonResponse  
    {
        $entityManager = $this->getEntityManager();
        $entity =$entityManager->getRepository(OpcionesEvaluacion::class)->find($id);
        if (!$entity) {
            return new JsonResponse(['msg'=>'No existen Registros con el id: '.$id],404);  
        }
   ...
   {
       $entityManager = $this->getEntityManager();
       $entityOpciones =$entityManager->getRepository(OpcionesEvaluacion::class)->find($id);
       if (!$entityOpciones) {
           return new JsonResponse(['msg'=>'No existen Registros con el id: '.$id],404);  
       }
