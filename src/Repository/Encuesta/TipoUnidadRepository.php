...
class TipoUnidadRepository extends ServiceEntityRepository
{
    ...
    public function findAllPage($data){
        ...
        foreach($paginator as $clave=>$valor){
            $tipounidadDto =new TipoUnidadOutPutDto();
    ...
    public function post($data,$validator,$helper): JsonResponse  {
        ...
        $entity=$helper->setParametersToEntity(new TipoUnidad(),$data);
    ...
    {
        $entityManager = $this->getEntityManager();
        $entity =$entityManager->getRepository(TipoUnidad::class)->find($id);
        if (!$entity) {
            return new JsonResponse(['msg'=>'No existen Registros con el id: '.$id],404);  
        }
