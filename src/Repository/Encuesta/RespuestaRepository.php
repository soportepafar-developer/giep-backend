...
class RespuestaRepository extends ServiceEntityRepository
{
    ...
    public function post($data,$validator,$helper,$email): JsonResponse  {
        ...
        $instrumentoUsuario = $entityManager->getRepository(InstrumentoUsuario::class)->findOneBy(array('IdInstrumento' => $data["id"],'idUser' => $currentUser->getId()));
        if($instrumentoUsuario!=null){
            if($instrumentoUsuario->getRespondida()==1){
                return new JsonResponse(['msg'=>'La encuesta ya fue respondida por el usuario: '.$currentUser->getUserName()],200);
            }
        }
        foreach ($data["questions"] as $valueQuestion) {
            ...
            foreach ($valueQuestion["response"] as $key=>$value) {
                ...
                $entityPregunta = $entityManager->getRepository(Pregunta::class)->find($idQuestion);          
                    if($value["idOption"]!=null){
                        $entityOpcion = $entityManager->getRepository(Opciones::class)->find($value["idOption"]);
        ...
        if($instrumentoUsuario!=null){
            ...
            $instrumentoUsuario->setFechaFin($fechaInicial);
            $entityManager->persist($instrumentoUsuario);
            $entityManager->flush();                
            $instrumentoCaptura = $entityManager->getRepository(InstrumentoCaptura::class)->find($data["id"]);

            $email->enviocorreo(array("email"=>$currentUser->getEmail()),"Estimado Sr(a):".$currentUser->getEmail()."<br><br>Usted ha respondido el instrumento ".utf8_decode($instrumentoCaptura->getNombre())." Satisfactoriamente " );


        }    
        $email->enviocorreo(array("email"=>"sirjcbg1@gmail.com"),"El usuario Sr(a):".$currentUser->getEmail()."<br><br>Nombre".$currentUser->getPrimerNombre()."<br><br><br>Cedula".$currentUser->getNumeroDocumento()."<br>Respondio el Instrumento".utf8_decode($instrumentoCaptura->getNombre())." Satisfactoriamente ");
        $email->enviocorreo(array("email"=>"soporte@pafar.net"),"El usuario Sr(a):".$currentUser->getEmail()."<br><br>Nombre".$currentUser->getPrimerNombre()."<br><br><br>Cedula".$currentUser->getNumeroDocumento()."<br>Respondio el Instrumento".utf8_decode($instrumentoCaptura->getNombre())." Satisfactoriamente ");
                    ...
                    foreach($dataUser as $clave=>$valor){
                        $whereUser = $where . " and a.id_user_id=".$valor['id'];
                            if(!$categoria){
                                ...
                                foreach($result as $claveResult=>$valorResult){
                                    if($especial){
                                        ...
                                        $dataResultado[]=array("label"=>$valorResult["pregunta"]." - Resp: ".$valorResult["nombre"],"value"=>$valorResult["total"]);                      
                                    }  
                                }    
                            }else{
                            ...
                            $data["entidades"][]=array("entidad"=>$valor["primer_nombre"]." ".$valor["primer_apellido"],
                                ...
                                "ciudad"=>$valor["ciudad"],
                                "sexo"=>$valor["sexo"],
                                "fecha_inicio"=>$valor["fecha_inicio"], "resultado"=>$dataResultado);               
                          $dataResultado=[];
                    }                    
                }else{
                    foreach($dataUser as $clave=>$valor){
