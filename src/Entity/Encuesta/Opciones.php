...
class Opciones
{
    ...
    /**
     * @ORM\OneToMany(targetEntity=OpcionesCargo::class, mappedBy="opcion")
     */
    private $opcionesCargos;

    /**
     * @ORM\ManyToOne(targetEntity=Empresa::class, inversedBy="idopciones")
     */
    private $idempresa;
...
//        $this->Cargo = new ArrayCollection();
    ...
    public function removeOpcionesCargo(OpcionesCargo $opcionesCargo): self
    {
        if ($this->opcionesCargos->removeElement($opcionesCargo)) {
            // set the owning side to null (unless already changed)
            if ($opcionesCargo->getOpcion() === $this) {
                $opcionesCargo->setOpcion(null);
            }
        }
